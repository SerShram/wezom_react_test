...
const CardsViewSwitch = ({typeOfCards, setTypeOfCards}) => {
  ...
  return (
    <div className="flex">
      <button className={(typeOfCards ? s.is_tiles : s.tiles) + ' flex flex-ai-c flex-jc-c'}
              onClick={() => setViewDateOfUsers(true)}>
      ...
      <button className={(!typeOfCards ? s.is_tabular : s.tabular) + " flex flex-ai-c flex-jc-c"}
              onClick={() => setViewDateOfUsers(false)}>
